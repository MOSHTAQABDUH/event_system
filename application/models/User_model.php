...
class User_model extends CI_Model {
    ...
    {     
        $this->db->get_where('users', array('phone' => $phone), 1);
    ...
    {   
        $this->db->where('id', $post['user_id']);
        $this->db->update('users', array('dpt' => $post['dpt'], 'email' => $post['email'], 'first_name' => $post['firstname'], 'company' => $post['company'], 'phone' => $post['phone'])); 
        $success = $this->db->affected_rows(); 
    ...
    {   
        ...
        if(!$success){
            error_log('Unable to updatePassword("0")');
            return false;
        }        
        return true;
    }
